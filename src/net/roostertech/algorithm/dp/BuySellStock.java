...
public class BuySellStock {
    ...
    public int maxProfitv1(final List<Integer> prices) {
        int priceCnt = prices.size();

        if (prices.size() < 2) {
            return 0;
        }

        // max profit before a index
        int[] left = new int[priceCnt];
        // max profit after a index
        int[] right = new int[priceCnt];


        left[0] = 0;
        int min = prices.get(0);
        for (int i = 1; i < priceCnt; i++) {
            min = Math.min(min, prices.get(i));
            left[i] = Math.max(left[i  - 1], prices.get(i) - min);
        }

        right[priceCnt - 1] = 0;
        int max = prices.get(priceCnt - 1);
        for (int i = priceCnt - 2; i >=0; i--) {
            max = Math.max(max, prices.get(i));
            right[i] = Math.max(right[i+1], max - prices.get(i));
        }

        // find a point that left + right is the highest value
        int maxProfit = 0;
    ...
    public int maxProfit(final List<Integer> prices) {
        ...
        /**
         * Calculate max profit if were to take certain action at a particular index
         * A buy would result in debit (negative) profit
         * A sell would result in credit (positive) profit
         * firstBuy - if the first stock was purchased before/at this point. This would be
         * debit of the lowest stock price till this point.
         * firstSell - best delta of firstBuy (lowest price till here) with prices till here
         * secondBuy - if second stock was purchased before/at this index.
         * secondSell - best delta of secondBuy and prices till here. This would be our final max profit
         */
        for(int i:prices){
